...
docker run --name neo4j -d -p 7474:7474 -p 7687:7687 -v neo4jdata:/data -v d:/neo4j_plugins:/plugins -e NEO4J_dbms_security_procedures_unrestricted=apoc.\\\* -e NEO4J_AUTH=neo4j/test neo4j
...
rem docker run --name mongo-express -d -p 8081:8081 --network="mongonetwork" --restart=always -e ME_CONFIG_MONGODB_ADMINUSERNAME=root -e ME_CONFIG_MONGODB_ADMINPASSWORD=example mongo-express
rem docker network create --driver bridge pgnetwork
rem docker volume create --name=postgresdata
rem docker run --name postgres -d -p 5432:5432 --hostname=postgres -v=postgres:/pgdata --env-file=pg-env.list --network="pgnetwork" crunchydata/crunchy-postgres:centos7-10.9-2.4.1
